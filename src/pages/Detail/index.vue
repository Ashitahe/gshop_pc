    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :defaulImg="skuImageList[selectedImg].imgUrl" />
          <!-- 小图列表 -->
          <ImageList :imgList="skuImageList" @setImg="setImgIndex" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSale, index) in spuSaleAttrList" :key="spuSale.id">
                ...
                <dd
                  v-for="item in spuSale.spuSaleAttrValueList"
                  :class="{ active: item.isChecked === '1' }"
                  :key="item.Id"
                  @click="setSaleAttr(item, spuSale, index)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  class="itxt"
                  v-model="skuNum"
                  @change="skuNum = skuNum = skuNum > 1 ? parseInt(skuNum) : 1"
                />
...
export default {
  ...
  created() {
    ...
    this.$store.dispatch("getProductDetailInfo", skuId);
  },
  computed: {
    ...mapGetters([
      "categoryView",
  ...
  methods: {
    ...
    setSaleAttr(item, spuSale, index) {
      const { spuSaleAttrValueList } = spuSale;
      spuSaleAttrValueList.forEach((element) => element.isChecked = "0");
      item.isChecked = "1";
    },
    addToCart() {
      ...
      this.$store.dispatch('changeShopCart',{skuId,skuNum})
      .then((result) => {
        // 成功则跳转
        // 用sessionStorage传递商品信息
        window.sessionStorage.setItem('SKUINFO_KEY',JSON.stringify(this.skuInfo));
        this.$router.push(locaton);
      }).catch((err) => {
        // 失败提示
