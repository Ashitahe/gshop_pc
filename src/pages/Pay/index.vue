...
import validate from "uuid/dist/validate";
export default {
  created() {
  ...
  methods: {
    async pay() {
      try {
        const res = await QRCode.toDataURL(this.codeUrl);
        this.$alert(`<img src="${res}">`, "请使用微信扫码支付", {
          dangerouslyUseHTMLString: true, // 将内容字符串作为html解析
          showClose: false, //显示右上角的关闭按钮
          showCancelButton: true, //显示取消按钮
          cancelButtonText: "支付遇到问题？", //取消按钮显示的文字
          confirmButtonText: "我已支付", //确认按钮显示的文字
          center: true, //居中布局
        })
          .then(() => {
            // 关闭提示框后停止查询定时器
          ...
          this.$API
            ...
            .then((value) => {
              if (value.code === 200) {
                ...
                // 清除购物车中的物品
                this.$store.dispatch("deleteCheckedCartItem");
              }
            })
            .catch((err) => {
              console.log("---", error);
        ...
        this.$message.warning("支付二维码生成失败");
      }
    },
  },
};
